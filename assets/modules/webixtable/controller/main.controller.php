...
/** ajax methods **/
        ...
        switch ($opetarion) {
            ...
            case 'insert':
                if (!empty($arr) && isset($arr[$idField]) && $arr[$idField] != '') {
                    ...
                    $max = $this->modx->db->getValue($this->modx->db->query("SELECT MAX(`" . $idField . "`) FROM " . $this->getTable()));
                    $max = $max ? ($max + 1) : 1;
    ...
    {
        $max = $this->modx->db->getValue($this->modx->db->query("SELECT MAX(`" . $this->getCfg('idField') . "`) FROM " . $this->getTable()));
